...
return gulp.src(lessSrc) //该任务针对的文件
    ...
    // 编译less
    .pipe(less()) //该任务调用的模块
    .pipe(minifyCss({ // 压缩css文件，减小文件大小，并给引用url添加版本号避免缓存
        compatibility: 'ie7'
    })) //压缩css
    .pipe(autoprefixer({//根据设置浏览器版本自动处理浏览器前缀
...
gulp.task('assetRev', function() {
    return gulp.src(cssMinSrc) //该任务针对的文件
...
gulp.task('serve', function() {
    ...
    gulp.watch('app/js/*.js', ['dev']); //监控文件变化，自动更新  
    gulp.watch('app/css/*.less', ['dev']);
    gulp.watch('app/views/*.html', ['dev']);
    gulp.watch('app/images/*.*', ['dev']);
});
